...
const outDir = path.join(repoRoot, "contracts", "MedalSweep3");
...
function contractName(k: number, epsilon: number) {
  return `MedalSweep3_k${k}_e${String(epsilon).replace(".", "_")}`;
}
...
async function main() {
  ...
  for (const r of results) {
    const artifactPath = path.join(repoRoot, "artifacts/contracts/MedalSweep3", `${r.name}.sol`, `${r.name}.json`);
